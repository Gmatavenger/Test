...
def load_secrets() -> bool:
    """Load secrets from .env file or prompt user if file is missing."""
    try:
...
# ------------------------------------------------------------------------------
USERNAME: Optional[str] = os.getenv("SPLUNK_USERNAME")
PASSWORD: Optional[str] = os.getenv("SPLUNK_PASSWORD")
session: Dict[str, Any] = {
    "dashboards": [],
    "username": USERNAME,
    "password": PASSWORD,
}
DASHBOARD_FILE: str = "dashboards.json"
# Use EST for folder naming and timestamps.
est = pytz.timezone("America/New_York")
SCREENSHOT_DIR: str = os.path.join("screenshots", datetime.now(est).strftime("%Y-%m-%d"))
os.makedirs(SCREENSHOT_DIR, exist_ok=True)
# For scheduled runs
scheduled: bool = False
schedule_interval: int = 0  # in seconds
# Define a default wait timeout for Splunk operations
SPLUNK_WAIT_TIMEOUT_MS: int = 90000  # Increased to 90 seconds for more robustness
# For Playwright automation
PLAYWRIGHT_HEADLESS: bool = True  # Set to False for debugging

# ------------------------------------------------------------------------------
# Signal & Exit Handling
# ------------------------------------------------------------------------------
def cleanup_and_exit(*_) -> None:
...
# Utility Functions
# ------------------------------------------------------------------------------
def load_dashboards() -> None:
    """Load dashboards from JSON file and ensure required fields exist."""
        ...
        try:
            with open(DASHBOARD_FILE, "r") as f:
                ...
                for dashboard in session["dashboards"]:
                    ...
                    if "group" not in dashboard:
                        dashboard["group"] = "Default"
            logging.info(f"Loaded {len(session['dashboards'])} dashboards from {DASHBOARD_FILE}")
        except json.JSONDecodeError:
            messagebox.showerror("Dashboard Load Error", "Error reading dashboard file. It may be corrupted.")
            logging.error("Failed to decode dashboard JSON file")
            session["dashboards"] = []
        except Exception as e:
            messagebox.showerror("Dashboard Load Error", f"Error loading dashboards: {e}")
            logging.error(f"Failed to load dashboards: {e}")
            session["dashboards"] = []
    else:
        session["dashboards"] = []
        logging.info("No dashboard file found, starting with empty dashboard list")

def save_dashboards() -> None:
    """Save dashboards to JSON file with proper formatting."""
    try:
        with open(DASHBOARD_FILE, "w") as f:
            json.dump(session["dashboards"], f, indent=2)
        logging.info(f"Saved {len(session['dashboards'])} dashboards to {DASHBOARD_FILE}")
    except Exception as e:
        messagebox.showerror("Dashboard Save Error", f"Error saving dashboards: {e}")
        logging.error(f"Failed to save dashboards: {e}")
...
def manage_credentials() -> None:
    """Display a dialog to manage user credentials."""
    ...
    new_password = simpledialog.askstring(
        ...
        "Enter new password:",
        show="*", 
        initialvalue=session.get("password", "")
    )
...
# ------------------------------------------------------------------------------
def add_dashboard() -> None:
    """Display a dialog to add a new dashboard."""
    ...
    if not url or not is_valid_url(url):
        messagebox.showerror("Invalid URL", "Dashboard URL must be a valid URL starting with http or https.")
        return
...
def delete_dashboard() -> None:
    ...
    for item_id in selected_items:
        name = treeview.item(item_id, "values")[1]  # Get dashboard name from the values tuple
...
def refresh_dashboard_list() -> None:
    """Refresh the dashboard list in the Treeview."""
...
def toggle_selection(event) -> None:
    """Toggle the selection state of a dashboard when its checkbox is clicked."""
...
def update_group_filter() -> None:
    """Update the group filter combobox with available groups."""
    groups = {"All"}
...
def get_selected_dashboards() -> List[Dict[str, Any]]:
    """Return a list of selected dashboards from the session data."""
    return [d for d in session["dashboards"] if d.get("selected", False)]

# ------------------------------------------------------------------------------
# Advanced Time Range Selection Using EST and Dropdown Options
# ------------------------------------------------------------------------------
def get_time_range() -> Tuple[Optional[str], Optional[str]]:
    """
    Display a pop-up to choose the time range in EST.
    Users can choose from several relative options or select "Custom Absolute".
    Returns a tuple of (start_time, end_time) or (None, None) if cancelled.
    """
    ...
    def toggle_abs_frame(event) -> None:
    ...
    def submit() -> None:
        try:
            ...
            if selection == "Last 1 Hour (EST)":
                ...
                if not validate_time_format(s_time) or not validate_time_format(e_time):
    ...
    popup.grab_set()
    popup.wait_window()
    return None, None  # Return None, None if window is closed without submitting

# ------------------------------------------------------------------------------
# Scheduled Analysis Functionality
# ------------------------------------------------------------------------------
def schedule_analysis() -> None:
    """Display a dialog to schedule regular analysis."""
...
def run_scheduled_analysis() -> None:
    """Run the scheduled analysis and schedule the next run."""
    if not scheduled:
...
def cancel_scheduled_analysis() -> None:
...
def analyze_selected() -> None:
    """Analyze selected dashboards based on user input."""
...
async def _wait_for_splunk_dashboard_to_load(page: Page, timeout_ms: int = SPLUNK_WAIT_TIMEOUT_MS) -> None:
    """
    Intelligently for Splunk dashboard panels to load their content.
    This includes:
    ...
    while (datetime.now() - start_time).total_seconds() * 1000 < timeout_ms:
        panels_state = await page.evaluate(f"""(loadingIndicators) => {{
            ...
            }} else if (panelseWithLoadingText.length > 0) {{
...
async def visit_dashboard(url: str, start: str, end: str) -> str:
    """
...
async def analyze_dashboards_async(urls: List[str], start: str, end: str) -> None:
    """
...
def show_results(messages: List[str], start: str, end: str) -> None:
    """Display analysis results in a new window with an option to export to HTML."""
    win = tk.Toplevel(app)
    ...
    def export_report() -> None:
        """Export the analysis report to an HTML file."""
...
def create_tooltip(widget, text):
    ...
    def enter(event):
        ...
        tooltip.bind("<Leave>", lambda e: leave())
...
control_buttons = [
    (add_btn, "Add a new dashboard to the list"),
    (delete_btn, "Delete selected dashboards"),
    (analyze_btn, "Analyze selected dashboards"),
    (schedule_btn, "Schedule regular analysis"),
    (cancel_schedule_btn, "Cancel scheduled analysis")
]

for btn, tooltip_text in control_buttons:
...
add_btn = tk.Button(control_frame, text="Add Dashboard", command=add_dashboard)
add_btn.grid(row=0, column=0, padx=5)
delete_btn = tk.Button(control_frame, text="Delete Dashboard", command=delete_dashboard)
delete_btn.grid(row=0, column=1, padx=5)
analyze_btn = tk.Button(control_frame, text="Analyze Selected", command=analyze_selected)
analyze_btn.grid(row=0, column=2, padx=5)
schedule_btn = tk.Button(control_frame, text="Schedule Analysis", command=schedule_analysis)
schedule_btn.grid(row=0, column=3, padx=5)
cancel_schedule_btn = tk.Button(control_frame, text="Cancel Scheduled Analysis", command=cancel_scheduled_analysis)
cancel_schedule_btn.grid(row=0, column=4, padx=5)
