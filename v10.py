# =================================================================================================
# Splunk Dashboard Automator - Enhanced Desktop Application
# =================================================================================================
#
...
#   - Advanced Time Selection: An integrated time range selector in the main UI, removing popups.
#   - Dual-Mode Capture:
...
#   - Advanced Scheduling: A built-in manager to create and view multiple, persistent
#     schedules to run analysis jobs automatically.
#
# =================================================================================================
...
class SplunkAutomatorApp:
    ...
    def _setup_ui(self):
        ...
        time_frame.grid(row=0, column=0, sticky="ns", padx=(0, 10), pady=5)
        self._setup_time_controls(time_frame)
    ...
    def _setup_time_controls(self, parent_frame):
        """Creates the time selection widgets directly in the main UI."""
        self.time_choice = tk.StringVar(value="preset")
        ttk.Radiobutton(parent_frame, text="Presets", variable=self.time_choice, value="preset", command=self._update_time_controls).pack(anchor="w")
        ttk.Radiobutton(parent_frame, text="Relative", variable=self.time_choice, value="relative", command=self._update_time_controls).pack(anchor="w")
        self.time_controls_frame = ttk.Frame(parent_frame, padding=(15, 5, 0, 0))
        self.time_controls_frame.pack(anchor="w")
        self._update_time_controls()

    def _update_time_controls(self):
        """Dynamically shows the correct time controls based on the radio button selection."""
        for widget in self.time_controls_frame.winfo_children(): widget.destroy()
        ...
        if choice == "preset":
            ...
            self.time_rel_amount = ttk.Entry(self.time_controls_frame, width=5); self.time_rel_amount.insert(0, "4"); self.time_rel_amount.pack(side=tk.LEFT)
            ttk.Label(self.time_controls_frame, text="Unit:").pack(side=tk.LEFT, padx=(5,0))
            self.time_rel_unit = ttk.Combobox(self.time_controls_frame, values=["minutes", "hours", "days"], state="readonly", width=8); self.time_rel_unit.set("hours"); self.time_rel_unit.pack(side=tk.LEFT)
    
    def _get_time_range_from_ui(self) -> Optional[Dict]:
        """Reads the time range from the main UI controls and returns it as a dictionary."""
        try:
            choice = self.time_choice.get()
            if choice == "preset":
                ...
                unit_map = {"minutes": "m", "hours": "h", "days": "d"}
                unit = unit_map[self.time_rel_unit.get()]
                return {'start': f'-{amount}{unit}', 'end': 'now'}
        except (ValueError, KeyError, TypeError) as e:
            messagebox.showerror("Input Error", f"Invalid time range input: {e}"); return None
        return None
    ...
    def _start_processing_job(self, capture_only: bool, schedule_data: Optional[Dict] = None):
        ...
        time_range_str_or_dict = schedule_data['time_range'] if schedule_data else self._get_time_range_from_ui()
        if time_range_str_or_dict is None: return # User cancelled or entered invalid time.
        
        # Ensure time_range is a dictionary
        if isinstance(time_range_str_or_dict, str):
            time_range = {'start': time_range_str_or_dict, 'end': 'now'}
        else:
            time_range = time_range_str_or_dict

        retry_count = 2 # Default for schedules. For manual analysis, we could ask the user.
    ...
    async def process_single_dashboard(self, playwright, dashboard_data: Dict, time_range: Dict, capture_only: bool):
        ...
        try:
            ...
            full_url = self.format_time_for_url(url, time_range, is_studio)
    ...
    def format_time_for_url(self, base_url: str, time_range: Dict, is_studio: bool) -> str:
        """Appends the correct time range parameters to the Splunk dashboard URL."""
        parsed_url = urlparse(base_url)
        query_params = parse_qs(parsed_url.query)
        prefix = "form.global_time" if is_studio else "form.time" # Studio uses a different prefix
